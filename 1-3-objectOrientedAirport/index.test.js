...
function createValidObjects() {
  return validObjects = {
    passengers: [
      ...
      new Passenger("gold pass 1", 4321, "F5", true, [new Bag(10), new Bag(18.7)]),
...
describe('Passenger', () => {
  ...
  test('the passenger should be able to add bags', () => {
    ...
    expect(() => {passengers[0].addBag(new Bag(5))}).toThrow(Error)
    expect(() => {passengers[2].addBag(new Bag(25))}).toThrow(Error)
  ...
  test('creation validation', () => {
    expect(() => new Passenger('mary sue', 333, 'E4', false, [new Bag(5), new Bag(20)])).toThrow(Error)
  })
...
describe('Plane', () => {
  ...
  test('the plane should only allow passengers to board', () => {
    expect(() => planes[0].board(5)).toThrow(Error)
  })
  
  test('the plane\'s capacity should decrease with each passenger', () => {
    const capacityBefore = planes[0].remainingCapacity
    planes[0].board(passengers[0])
    expect(capacityBefore).toBe(planes[0].remainingCapacity + 1)
  })

  test('the plane will reject passengers when at capacity', () => {
    const plane = new Plane('boeing', 0)
    const deniedPassenger = new Passenger('name', 1, "2A", false)

    expect(() => plane.board(deniedPassenger)).toThrow(Error)
